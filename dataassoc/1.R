...
transcations = read.transactions("genre.txt", header= FALSE)
...
sort(freq_transcations, decreasing= TRUE)
# 6.
...
# the minimum support = 0.01, minimum confidence = 0.25, minimum cardinality (number of items in the rule) = 2. Use the function apriori
#help("apriori")
# support used to produce a large number of rules
# confidence 0.25 to boost the effect to the possiblity of getting better lift
# only 2-3 genres are accepted in a rule, more than that is too much 

rules <- apriori(transcations, parameter= c(support= 0.01, confidence= 0.25, minlen= 2, maxlen= 3))
